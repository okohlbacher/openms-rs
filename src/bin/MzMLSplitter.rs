...
use openms::cli::{ExitCode, Tool, ToolContext, ToolSpec};
...
impl Tool for MzMLSplitter {
    ...
    fn run(ctx: &ToolContext) -> Result<ExitCode> {
        ...
        if out.is_empty() {
            out = openms::format::file_types::strip_extension(&input).to_owned();
        }
...
fn main() -> std::process::ExitCode {
    std::process::ExitCode::from(openms::cli::run::<MzMLSplitter>().as_i32() as u8)
}
