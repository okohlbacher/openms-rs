...
`format::idxml` reads and writes identification results using the existing Rust identification types. It targets the idXML 1.5 representation in OpenMS4-core revision `7c029e8cdba6abab503708ecdd56f6ab55e38ce4`. The optional `idxml` feature is enabled by default and shares the `quick-xml` dependency with mzML. It calls no C++ code.
...
- Tool-defined `modification_definitions` registration or peptide modifications unsupported by the native sequence parser. The adapter does not reinterpret custom definitions using a similarly named built-in modification.
...
writer call. A missing record or same-name record with different chemistry is an
error. Reading the output requires equivalent registry contents; the XML does not
embed or manufacture a chemical database. Existing read/write entry points use
the immutable global registry. Anonymous tags retain their normal bracket syntax.
