...
The source writer-option group now consumes the existing Float `selected ion m/z` discriminator in every ordinary/Numpress/source-option writer, emits it as the selected-ion CV, and omits it from activation metadata. When this discriminator is present, the effective `Precursor.mz` is preserved in an isolation window even without offsets or a separate retained target. Default-mode rereading therefore recovers selected m/z plus a separate isolation target; isolation-mode rereading recovers effective target m/z plus the Float discriminator. Explicit TPP compatibility intentionally suppresses the isolation window while retaining the selected-ion value.

The source literal target500/selected499.5 fixture and synthetic target250/selected999 spectrum/chromatogram both exercise all writer families and both reader modes. See [writer support](MZML_WRITE_OPTIONS_SUPPORT.md) for precision, indexed output, and the checked native representation boundaries.
