...
When `store_original_rt` is enabled, scalar records gain `original_RT` only if that key is absent. Existing values are retained on repeated transformations even when their type or contents differ from a newly generated value. Peptide metadata stores a typed floating value. The current spectrum/feature kernel metadata uses string maps, so these scalar values use round-trippable decimal strings. Chromatograms use the source's lower-case key `original_rt` and preserve the entire original time vector as a string such as `[1, 3]`; empty chromatograms store `[]`. This string encoding reflects the current kernel metadata representation and is not a typed C++ `DataValue` list.
