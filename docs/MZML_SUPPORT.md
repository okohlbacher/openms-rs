...
Precursors attach directly to `MSSpectrum` and `MSChromatogram`. Isolation target (`MS:1000827`) and lower/upper offsets (`MS:1000828`/`1000829`) use m/z units. A missing selected m/z falls back to the isolation target in default selected-ion mode. [Isolation-target loading](MZML_ISOLATION_SUPPORT.md) instead retains the target as effective m/z, stores a differing selected value in typed precursor metadata and filters on target events. Its current writer round-trip boundary is explicit. When target and selected m/z agree, reading normalizes `isolation_target_mz` to `None`; the numeric target is retained implicitly, without preserving whether the source explicitly wrote it. A different target remains `Some(value)`. Writing rejects a negative effective target whenever an isolation window is emitted, including fallback from a signed selected m/z. Algorithms follow the source's selected-m/z convention when defining purity windows.
