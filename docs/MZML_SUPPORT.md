...
`spectrumRef` is retained as `Precursor::spectrum_reference`. Reading permits unresolved references for filtered input; parent lookup can then use acquisition-order fallback. Writing emits the reference as it stands, whether or not it names a spectrum of the output document, as source `MzMLHandler::writePrecursor_` does (`MzMLHandler.cpp:4535-4547` at core bc9cc12: it writes the `spectrum_ref` meta value verbatim and resolves nothing). The writer required resolution until the benchmark of 2026-09-15 showed that this makes `MzMLSplitter` unusable — the tool moves whole spectra into parts, so every MS2 whose precursor stayed behind carries a reference out of its part, and the C++ tool writes those parts (its part 2 of 4 for the 1.2 GB LTQ Orbitrap Velos run carries four references to `scan=10929`, which part 1 holds). The reference is still required to be a well-formed XML string. Note that mzML 1.1's `xs:keyref KEYREF_PRECURSOR_SPECTRUMREF` does demand resolution (`share/OpenMS/SCHEMAS/mzML_idx_1_10.xsd`), so split parts are schema-invalid on both implementations. Writing does not enforce parent MS level or order as an XML constraint. Duplicate isolation/activation containers, duplicate supported scalar fields, conflicting mobility quantities and misplaced containers are rejected.
...
`read_with_options` accepts `ReadOptions`. Every **cumulative** ceiling is size-derived: `floor + rate * consumed XML bytes`, configured by `ReadOptions::scaling` ([`InputScaling`](MZML_READER_SCALE_SUPPORT.md)), and applies together with the absolute `max_*` ceiling of the same quantity, which now defaults to unbounded. The floors are the former fixed ceilings, so a small document is bounded exactly as before: 10 million total peaks, 512 MiB decoded array bytes, 20 million array elements, 10 million combined groups/parameters/reference occurrences, 512 MiB cumulative conservative parameter bytes, 1 million records, 1 million binary arrays and 100,000 parameter groups. These include primary arrays; empty string elements count toward the element limit, while empty placeholders count toward the array limit. These bounds cover annotation storage independently of peak count, and include unused definitions and ignored-header expansion. Counts that need their own start tag — records, binary arrays and parameter groups — grow once per group of consumed bytes (one record per 512, one array per 256, one group per 4,096) rather than per byte. The two per-item ceilings stay absolute and do **not** grow with the document: 1 TiB of XML input, and 64 MiB of compressed or decoded binary bytes per array, which is 155 times the largest array in any benchmark input. `InputScaling::fixed()` restores the former fixed behaviour. The defaults were measured against the multi-gigabyte benchmark inputs; see [reader scale](MZML_READER_SCALE_SUPPORT.md).

XML nesting is limited to 128 levels. Base64 expansion has a corresponding bound. [The whitespace option](MZML_NORMALIZATION_SUPPORT.md) controls only normalization before binary decoding; XML and checked Base64 validation remain active. Zlib decoding uses bounded 8 KiB chunks with overrun detection and requires a complete stream with matching checksum and full input consumption; trailing or concatenated compressed data is rejected. Numeric bytes must match the declared count times precision exactly. Variable-size string data is checked against per-array and cumulative byte budgets, then against the exact declared element count. Adjust limits deliberately for an unusual document; the returned experiment still needs memory for all peaks, string objects and retained metadata.

The reader supports [referenceable parameter groups](MZML_PARAM_GROUPS_SUPPORT.md) through the same handlers as inline parameters, including grouped CV and user parameters. Missing references and malformed groups are errors. The reader supports [Numpress binary transport](MZML_NUMPRESS_SUPPORT.md), including source float64 precision repair. Outside those documented repairs, it rejects integer/string primary peak arrays, unimplemented semantic auxiliary CV types, other binary CV terms or codecs, other time units, nonfinite values, unsupported XML encodings, DTDs, CDATA, and entity references in element text. Auxiliary scalar metadata, represented units and processing references are retained by the [header codec](MZML_HEADER_SUPPORT.md). Unit attributes attached to precision/compression terms remain invalid. Standard XML escaping and numeric character references in attribute values are supported. Multiple selected ions within one precursor and duplicate userParam keys cannot be represented and are rejected. Conflicting or duplicate supported scientific CV values, duplicate records, missing arrays and incorrect array lengths are also errors. A declared list `count` that disagrees with the actual children is not; see below.

### Declared list counts are advisory on reading

Every mzML list element carries a `count` attribute. The reader **requires it and requires it to be numeric**, but a value that disagrees with the actual number of children is advisory: the record, binary-array, precursor, selected-ion, scan, scan-window and product lists all load unchanged, and the experiment is built from the children that are actually present.

This matches `MzMLHandler.cpp`, which reads the attribute in exactly four places and compares it with nothing:

| List | What the source does with `count` | Reading policy here |
| --- | --- | --- |
| `spectrumList` | progress range plus `reserveSpaceSpectra` (`MzMLHandler.cpp:965-979`) | advisory |
| `chromatogramList` | progress range plus `reserveSpaceChromatograms` (`:996-1013`) | advisory |
| `binaryDataArrayList` | `bin_data_.reserve(...)` (`:1015-1017`) | advisory |
| `selectedIonList` | warns when it exceeds one (`:1371-1375`) | advisory |
| `precursorList`, `productList`, `scanWindowList` | no open-tag handler; never read | advisory |
| `scanList` | read only as a parent tag (`:2259`, `:3496`); count never read | advisory |
| header lists (`sourceFileList`, `softwareList`, `dataProcessingList`, `componentList`, …) | never compared | advisory, see [header support](MZML_HEADER_SUPPORT.md) |
| `referenceableParamGroupList` | no handler; never read | **still compared** — a documented divergence, kept so that `read` and `read_size` agree, because the [counting reader](MZML_COUNTS_SUPPORT.md) rejects the same mismatch |

Real files carry wrong counts, and rejecting them cost the port twice. The TOPP fixture `DTAExtractor_1_input.mzML` declares `softwareList count="5"` with four entries; the upstream class-test input `MzMLFile_1.mzML` declares `<binaryDataArrayList count="2">` with four `binaryDataArray` children and `<productList count="1">` with two products, and `MzMLFile_test.cpp` loads it. Both are now read.
