...
`read_with_options` accepts `ReadOptions`. Every **cumulative** ceiling is size-derived: `floor + per_byte * consumed XML bytes`, configured by `ReadOptions::scaling` ([`InputScaling`](MZML_READER_SCALE_SUPPORT.md)), and applies together with the absolute `max_*` ceiling of the same quantity, which now defaults to unbounded. The floors are the former fixed ceilings, so a small document is bounded exactly as before: 10 million total peaks, 512 MiB decoded array bytes, 20 million array elements, 10 million combined groups/parameters/reference occurrences and 512 MiB cumulative conservative parameter bytes. These include primary arrays; empty string elements count toward the element limit, while empty placeholders count toward the array limit. These bounds cover annotation storage independently of peak count, and include unused definitions and ignored-header expansion. The two per-item ceilings stay absolute: 1 TiB of XML input and 512 MiB of compressed or decoded binary bytes per array. Counts that need their own start tag — records, binary arrays and parameter groups — are bounded by the input size itself and keep only their absolute ceiling, which defaults to unbounded. `InputScaling::fixed()` restores the former fixed behaviour. The defaults were measured against the multi-gigabyte benchmark inputs; see [reader scale](MZML_READER_SCALE_SUPPORT.md).
