...
`MetaValueData` represents the seven DataValue alternatives: `Empty`, `String`, `Integer(i64)`, `Float(f64)`, `StringList`, `IntegerList` and `FloatList`. `MetaValue` owns a private, validated alternative and an optional `Unit`. Integers/strings and their lists support `From`; floating scalars/lists use `TryFrom` to reject NaN and infinities. `MetaValue::new` provides the enum-based constructor. A crate-internal algorithm port whose executed source stores a non-finite `DataValue` (so far only `FeatureFinderAlgorithmPicked`'s `FWHM`, fit-score and `EGH_*` values; the `FWHM` becomes infinite when the `float` width overflows) stores it through a crate-private constructor without that check (accepted by lead decision D13 of wave 5; the same constructor stores the debug seed scores, which are always finite, to follow `setMetaValue(float)`); `validate`, `validate_meta` and every checked consumer still report such a value, and the public constructors keep rejecting it. Unsigned uint64 conversion is checked against int64 range. No mutable reference to stored floating data is exposed.
