...
`CompletionTime` is a validated Gregorian wall-clock timestamp, parsed from `YYYY-MM-DD HH:MM:SS` or the equivalent `T` separator and displayed in the source-style space form. It supports years 1–9999 and leap years, rejects leap-second 60 and does not infer a time zone. Absence is `None`. This is a bounded replacement for the optional timestamp field, not a port of general OpenMS DateTime arithmetic, locale parsing or timezone handling.
