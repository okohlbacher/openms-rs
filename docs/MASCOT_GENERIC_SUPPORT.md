...
| **A non-positive `MSLEVEL` is refused** rather than stored, unless `ReadOptions::source_ms_level` is set. | The source assigns whatever `std::stoi` returned, and MS level 0 makes the record invalid for every consumer. `source_ms_level` (A8) stores it as the source does, `0` as `0` and a negative value wrapped into the source's `UInt` (`-1` is `4294967295`), because the FileInfo report prints that value; the Release build's report for `MSLEVEL=-1` is reproduced byte for byte in `tests/file_info_a8.rs`. |
