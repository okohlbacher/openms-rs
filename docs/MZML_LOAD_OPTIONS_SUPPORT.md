...
The function supports [metadata-only early stopping](MZML_HEADER_SUPPORT.md), disabled array population and [consumer/transform loading](MZML_CONSUMER_SUPPORT.md). It rejects `skip_xml_checks = true` before reading input. [Isolation-target precursor selection](MZML_ISOLATION_SUPPORT.md) is implemented for `precursor_mz_selected_ion = false`; that document distinguishes source event order, native metadata and the outstanding writer boundary. Other outstanding `MzMLFile` operations remain separate work. The preexisting native selected-ion alias `MS:1000040` executes the same native filter as `MS:1000744`; source's selected-ion branch names the latter.
