...
The function supports [metadata-only early stopping](MZML_HEADER_SUPPORT.md), disabled array population and [consumer/transform loading](MZML_CONSUMER_SUPPORT.md). `skip_xml_checks = true` skips only the four-XML-whitespace removal before Base64 decoding; XML legality, alphabet, lengths and resource checks remain active. See [normalization support](MZML_NORMALIZATION_SUPPORT.md). [Isolation-target precursor selection](MZML_ISOLATION_SUPPORT.md) is implemented for `precursor_mz_selected_ion = false`; that document distinguishes source event order, native metadata and the integrated selected-ion writer behavior. Other outstanding `MzMLFile` operations remain separate work. The preexisting native selected-ion alias `MS:1000040` executes the same native filter as `MS:1000744`; source's selected-ion branch names the latter.
