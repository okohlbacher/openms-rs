# Chromatogram Product transport

The `mzml` feature reads and writes `MSChromatogram::product` through the existing `mzml::read` and `mzml::write` APIs. This completes transport of the target m/z produced by native extracted-ion chromatograms. It also preserves asymmetric isolation offsets and representable Product metadata.
...
Only one Product is represented per chromatogram. Duplicate containers, multiple isolation windows, repeated supported quantities, duplicate metadata names, and misplaced Product children are errors. Referenceable groups apply through exactly the same handlers, duplicate detection, parameter-count and byte accounting as inline values. Read errors return no partial experiment. Full writer preflight runs before the first output call; transport failures may still leave partial output. Spectrum Product vectors remain unrepresented: a nonempty spectrum product list raises `Unsupported` instead of silently discarding its contents.
...
[Provenance](../tests/data/mzml_product_provenance.json) pins all inspected source paths to `54a232fe2cae9c590d5c997fa49d20e7769860fb`. The handler, Product definition, schema and data fixture are unchanged from the preceding SDK pin; the class test changed only an unrelated string loop variable. See [general mzML support](MZML_SUPPORT.md) and [parameter groups](MZML_PARAM_GROUPS_SUPPORT.md) for the remaining adapter boundaries and configurable limits.
