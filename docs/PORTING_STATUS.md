...
Wave 1 of that bundle and the first crate wave are integrated (2026-09-14):
PeakTypeEstimator, FAIMSHelper, mzML spectrum mobility with FileHandler type
detection, the FeatureFinderAlgorithmPicked helper structures, source-precision
isotope patterns, the numeric text formatting FileInfo prints, the TOPPBase
lifecycle closure and FuzzyDiff test support. The
[work packages](EARLY_TOPP_WORK_PACKAGES.md#wave-1-status) list what is merged
and what is still open.
Wave 2 is integrated too (2026-09-14): the PeakPickerHiRes and median
signal-to-noise parameter contract with its fidelity fixes, source-compatible
dangling mzML references, TOPPBase lifecycle part 2 (`-write_ini` parity, format
checks, usage on stderr and the DataProcessing retrofit), the FileInfo library
preview, the FeatureFinderAlgorithmPicked front half up to seed selection,
`IMDataConverter::splitByFAIMSCV` and FeatureOverlapFilter with its quadtree. The
Levenberg-Marquardt crate failed its gate, so the Eigen transcription stays; see
[wave 2 status](EARLY_TOPP_WORK_PACKAGES.md#wave-2-status).

Wave 3 is integrated as well (2026-09-15): the Gauss and EGH trace fitters, the
FeatureFinderAlgorithmPicked feature stage, so `run()` produces features end to
end, three new TOPP tools (`PeakPickerHiRes`, `FileInfo` and
`FeatureFinderCentroided`), the Boost.Regex facade, a `-threads` wiring that
reaches the five wave-2 tool bodies through a scoped pool (the three new tools
are serial and not wired yet), size-derived mzML reader ceilings with the source's
timestamp leniency and its lossy minute conversion of a 32-bit time array
(`ReadOptions::source_time_array_precision`, the third source-compatibility
switch beside `source_dangling_references` and `source_invalid_timestamps`),
per-record writer budgets with `indexedmzML` by default, the
morphological filter's single-sample-element end behaviour, and a
Levenberg-Marquardt solver that matches the Linux x86_64 Release Eigen bit for
bit on all 141 traced trace fits. [BENCHMARKS](BENCHMARKS.md) records the first
...
Wave 6 is integrated (2026-09-18) and closes the last deferral of the early TOPP
bundle. `FeatureFinderCentroided` now processes FAIMS input: one algorithm run
per compensation voltage, the `FAIMS_CV` annotation and the corrected
cross-voltage merge, pinned per voltage group against the C++ Release build run
on that group alone ([details](TOPP_FEATURE_FINDER_CENTROIDED_SUPPORT.md)). That
is decision D5 settled for this tool; `IMDataConverter` stays partial for the
members D5 left out. The same wave takes the build-flag decision wave 5 left
open: x86_64 builds set `-C target-feature=+fma` from the tracked
`.cargo/config.toml`, which removes the 21 % one-thread cost the bit-exact glibc
`exp`, `log` and `powf` pay on a baseline target, changes no output, and raises
the minimum processor to Intel Haswell or AMD Piledriver — a **breaking runtime
requirement**, with a startup CPUID check that refuses an older processor with
exit 12 and the exact rebuild command instead of dying on `SIGILL`
([FMA_BUILD_FLAG](FMA_BUILD_FLAG.md)). See
[wave 6 status](EARLY_TOPP_WORK_PACKAGES.md#wave-6-status).
...
**Third-party dependencies are replaced by crates, not by our own code.** Wherever OpenMS relies on an external library — Boost, Eigen, evergreen, libcurl and similar — the port first looks for a suitable replacement crate, and writes its own implementation only when none is suitable. Code OpenMS implements itself is ported as before. A crate is suitable when it builds under the crate's MSRV of 1.85, is pure Rust (or C with a stated reason), is maintained and widely used, reproduces the upstream class-test expectations, and gives the same results on every machine. The last two are checked by measurement: a numerical crate whose algorithm lands outside a class test's asserted tolerance is not suitable, and a crate that picks SIMD code by CPU is used in its deterministic mode if it has one. `rustfft` is the worked example — its default planner was measured to differ from its scalar planner in 92-99% of output components for at most a 1.1x speedup on the lengths used here, so the port uses `FftPlannerScalar`. This reverses earlier practice, under which the port reimplemented an HTTP client and an FFT that `ureq` and `rustfft` provide. Every decision, including those where no crate was suitable, is recorded with its measurements in [THIRD_PARTY_CRATE_DECISIONS](THIRD_PARTY_CRATE_DECISIONS.md). Dependency notes: besides `rustfft`, crate wave 1 landed `statrs` (MultipleTesting's normal quantile) and `rand_mt` (the MT19937-64 engine of decoy generation and unique IDs), and moved reference expansion in the shared ID and map XML reader, the network URL-host check and two calendar-day checks onto `quick-xml`, `ureq`'s `http::Uri` and `chrono`. `fancy-regex` now carries the Boost.Regex facade in `src/concept/boost_regex.rs`, which landed after six review rounds: every expression it compiles gives Boost's answer, or construction refuses it with `Error::Unsupported`, measured over 6,531,674 compared cases from 134,075 patterns with 0 mismatches against a driver compiled from Boost 1.92, and with 0 refusals in all sixteen OpenMS-derived pattern families; consumers keep their per-pattern scanners until a consumer lane switches them. `levenberg-marquardt` with `nalgebra` was measured by package B3-LM and not adopted: Eigen's evaluation accounting is reproduced exactly, but the fitted parameters are further from the executed C++ than the transcription's, at 8,627 of 29,004 budgets once lane B3b had made the transcription bit-faithful to the Linux x86_64 Release Eigen on all 141 traced fits. The transcription stays, and the two pins moved to `[dev-dependencies]`, where the recorded candidate test is their only user. Boost's digamma stays hand-rolled, because the `special` crate was measured further from the executed libOpenMS than the port's series; `special` is no longer a dependency.
...
| FORMAT/TextFile, CsvFile, ParamXMLFile | `format::{TextFile,CsvFile,paramxml}` | Source literal text/CSV helpers and optional INI transport through 1.8.0, with the source's ISO-8859-1 declaration as a writer option; [text/CSV](TEXT_CSV_SUPPORT.md), [INI](PARAMXML_SUPPORT.md) |
...
| DATASTRUCTURES/ConvexHull2D, DBoundingBox (2D) | `ConvexHull2D`, `Point2D`, `BoundingBox2D` | Source scan-envelope containment, outline preservation, compression and rectangular bounds; not a general convex-polygon implementation. `DBoundingBox.h` is partial: two dimensions with `intersects`, `width` and `height` ([details](ISOTOPE_SOURCE_PRECISION_SUPPORT.md)). Merging m/z at one retention time uses `f64::min`/`f64::max`, so signed-zero m/z differs from the source's keep-first rule (open kernel follow-up) |
...
| CHEMISTRY/ISOTOPEDISTRIBUTION | `IsotopeDistribution`, `CoarseIsotopePatternGenerator`, `FineIsotopePatternGenerator`, `FineIsotopeIterator` | Coarse convolution/enrichment/averagine/conditional fragments plus bounded fine configurations, source thresholds and coverage, ordered raw streams and custom binary64 populations; [coarse](ISOTOPE_SUPPORT.md) and [fine](FINE_ISOTOPE_SUPPORT.md). `CoarseIsotopePatternGenerator.h` and `IsotopeDistribution.h` are partial, with opt-in source binary32 precision and the source `trimLeft` ([source precision](ISOTOPE_SOURCE_PRECISION_SUPPORT.md)); that precision matches the SDK bit for bit only in runs that iterate elements in the port's order, the majority order for natural elements, and never for a formula containing iridium, which the SDK builds from rhenium's tables |
...
| PROCESSING/CENTROIDING/PeakPickerHiRes | `PeakPickerHiRes`, `CubicSpline2d`, `SignalToNoiseEstimatorMedian` | Partial: spectrum/chromatogram centroiding, S/N, spacing/missing-flank rules, FWHM, boundaries and experiment selection, with the source parameter contract (`defaults`, `from_param`, `to_param`), `get_type(true)` experiment selection and `PickingCompatibility` for the source's acceptance of degenerate input; tier 1 on 93 executed cases bit for bit. The `PeakPickerHiRes` TOPP tool now drives it in memory, and the input-derived acquisition ledger and `pick_experiment_in_place` let it centroid a 2.3 GB instrument run whose 22,776,198 centroids are bit-identical in m/z and intensity to the C++ Release tool's, and whose picked TIC chromatogram is bit-identical too once the reader reproduces the source's `f32` narrowing of a converted 32-bit time array (`CPP-306`, `fix/picked-chromatogram`). The Mobilogram and on-disc overloads remain; `-processOption lowmemory` is ported (P4-PICKER-LOWMEM), including the index the mode writes, its stored-type-only automatic mode, the source's dangling header references and the rest of the mode's divergence from the in-memory one, with the reader reading those references back under the same source-compatibility switch (decision D14, `fix/reader-round-trip`), and measured at 81.7 MiB resident against 3.29 GiB on a 2.3 GB instrument run; the estimator headers `SignalToNoiseEstimatorMedian.h` and `SignalToNoiseEstimator.h` are complete ([details](SIGNAL_TO_NOISE_SUPPORT.md)); [details](PEAK_PICKING_SUPPORT.md), [tool](TOPP_PEAK_PICKER_HI_RES_SUPPORT.md) |
| PROCESSING/CENTROIDING/PeakPickerIterative | `processing::iterative` | HiRes seeds, iterative raw-sample refinement, original-seed suppression priority, integrated intensities, exact regions and rounded source arrays; carries a `PickingCompatibility` (decision D15) — see the support document for which flags are effective and why `allow_unsorted_positions` is inert; [details](ITERATIVE_PICKING_SUPPORT.md) |
...
| ANALYSIS/OPENSWATH/PeakPickerChromatogram | `processing::chromatogram` | Legacy/corrected picking, smoothing, independent seed/boundary S/N, source overlap adjustment, raw sums and exact input regions; carries a `PickingCompatibility` whose boundary noise estimate follows the source unconditionally (decision D15) — see the support document for which flags are effective and why `allow_unsorted_positions` is inert; [details](CHROMATOGRAM_PICKING_SUPPORT.md) |
...
| FORMAT/FileHandler | `format::FileHandler` | Native experiment/feature/consensus dispatch, allowed types, bounded common content detection, gzip/bzip2 transport and atomic path output; `get_type` and the option-taking experiment and feature loaders ([details](MZML_MOBILITY_SUPPORT.md)); other source dispatch families remain |
| FORMAT/FeatureXMLFile, ConsensusXMLFile | `format::featurexml`, `format::consensusxml` | Recursive feature geometry, source filters, typed metadata, processing history, portable identification chemistry and consensus group quantities; featureXML writes and reads the source's `inf`/`-inf`/`NaN` spellings for feature values and float metadata; [featureXML](FEATUREXML_SUPPORT.md), [consensusXML](CONSENSUSXML_SUPPORT.md); ZIP input and inherited XSD validation remain |
...
| FORMAT/MzMLFile | `format::mzml` | Optional bounded subset, including spectrum- and scan-level mobility and unit-bearing ion-mobility arrays ([mobility](MZML_MOBILITY_SUPPORT.md)) and opt-in source reading of dangling software, data-processing and source-file references ([headers](MZML_HEADER_SUPPORT.md)); see [detailed support](MZML_SUPPORT.md) |
| FORMAT/FileInfo | `format::file_info::{FileInfo, Options, FileInfoResult}` | Partial: the result model, `run`, `toText` and `toTSV`, with the DTA, DTA2D, mzML and featureXML branches and `-m`, `-p` and `-s`, tier 1 on 34 executed report pairs; `-i`, `-d` and `-c` are ported (A6-FILEINFO), tier 1 on 59 executed Release-build cases of which 38 compare both reports byte for byte; the consensusXML, idXML/mzIdentML and FASTA branches are ported (A7-FILEINFO), tier 1 on 75 executed Release-build cases of which 53 compare both reports byte for byte, refusing only the three places the source's behaviour is an out-of-bounds `std::vector` access (`CPP-341`, `CPP-342`, `CPP-343`) and deferring one where it sorts a NaN-bearing sample (`CPP-347`); `-v` and the pepXML, mzTab, trafoXML and PQP branches, and the mzXML, mzData, MGF, MS2, sqMass, XMass and MSP peak files, stay refused until package A8. The `FileInfo` TOPP tool wraps it, with tier-1 evidence from ten reproduced upstream registrations, nine of them compared with a retained output, and 17 executed product-SDK cases; [details](FILE_INFO_SUPPORT.md), [A7](FILE_INFO_A7_SUPPORT.md), [tool](TOPP_FILE_INFO_SUPPORT.md) |
| FORMAT/PeakTypeEstimator | `format::peak_type_estimator::PeakTypeEstimator` | Complete: the source estimator on a `Peak1D` slice, shared with `MSSpectrum::get_type(true)`, tier 1 against the executed SDK; non-finite input refused; [details](PEAK_TYPE_ESTIMATOR_SUPPORT.md) |
| IONMOBILITY/FAIMSHelper | `kernel::faims_helper::FaimsHelper` | Complete: compensation-voltage set and FAIMS-CV identification filter, every class-test section including the end-to-end reader section, tier 1 against the executed SDK; infinite targets filtered as the source filters them; NaN voltages, a NaN target and impossible tolerances refused; [details](FAIMS_HELPER_SUPPORT.md) |
| DATASTRUCTURES/StringUtils, ListUtilsIO; CONCEPT/Types `writtenDigits` | `format::file_info::text_format` | Partial, FileInfo scope: `number`, full-precision `toStr` with `std::to_chars` ties to even, integer and string `toStr`, vector `operator<<` and stream `%g` text, tier 1 against the executed SDK, with platform differences documented; low-precision and `long double` forms, `numberLength`, `VecLowPrecision` and the other string utilities remain; [details](../src/format/file_info/text_format.rs) |
| FEATUREFINDER/FeatureFinderAlgorithmPicked | `analysis::feature_finder_picked::{algorithm, debug, defs, extension, fitting, instance, resolution, scoring, seeds, source_sort}`, plus the crate-private `glibc_powf` and `glibc_libm` | Complete: the whole algorithm is ported and `run()` produces features end to end — parameters, input checks, the scores, pattern precalculation, seed selection, mass-trace extension, the isotope fit, the trace fits, the quality checks, cropping, abort bookkeeping and overlap resolution. Tier 1: the seed stage bit for bit in six of seven executed configurations, and the feature stage against the C++ Release build (8 features, 30 hulls and 120 hull points in the default `FeatureFinderCentroided_1` run, 24 seeds in its `-seeds` run and 1,054 hull points under `-debug 5`, bit-identical m/z and hull points, fitted fields bit for bit on every platform (the EGH area's atan on a host other than x86_64 Linux with glibc: a measured maximum)). `min_spectra = 1` now follows the source and a changed isotope abundance computes the intended override, per the lead's decisions; the reusable instance (caller maps, accumulated aborts, parameter surface, progress), `write_debug` with `writeFeatureDebugInfo_` and `FeatureFinderDefs` are ported; degenerate bins, short and non-finite input, every source sort (introsort and libstdc++ 14.4.0 `stable_sort`, NaN and equal keys), the area iterator's drift-time filter, the step-2.5 `length_error` (with its debug side effects and the tool's exit 12), 64-bit integer parameters and the reference build's glibc `powf` follow the Linux x86_64 Release build, as do underflowed isotope windows, a NaN optional cutoff, in-bounds `UInt` wraps, the Pearson correlation's zero denominator, the stored non-finite widths and meta values and the reference build's glibc `exp` and `log` in both trace fitters, all tier 1 against it (lead decisions D1-D13 of wave 5); refused only where the source has no reproducible answer, each refusal where the C++ process ends recording where and how, with the length at which it leaves `debug/log.txt` (out-of-bounds reads and writes such as an empty best isotope pattern or a wrapped charge count, the endless loop, termination (the step-3.3.5 exception, executed; a charge-0 remainder; a stale abort seed), address-dependent output); the multi-thread abort race gives the single-thread result (D11); [details](FEATURE_FINDER_PICKED_SUPPORT.md) |
| FEATUREFINDER/TraceFitter, GaussTraceFitter, EGHTraceFitter | `analysis::feature_finder_picked::{trace_fitter, gauss_trace_fitter, egh_trace_fitter}` | Complete: every public and protected member of all three, with the shared start-value steps, the `Param` mapping and the gnuplot number text. Tier 3 for the 51 class-test sections (16 `TraceFitter_test` + 17 `GaussTraceFitter_test` + 18 `EGHTraceFitter_test`), tier 1 against the executed product SDK and the C2 oracle: start values, residuals, Jacobians and queries bit-identical, fits and budget sweeps within 1e-9, statuses, `nfev`, `njev` and the budget boundaries exact. Known and documented: the fits follow the Linux x86_64 Release Eigen rather than the macOS arm64 oracle that produced the fixtures; [details](TRACE_FITTER_SUPPORT.md) |
| IONMOBILITY/IMDataConverter | `kernel::im_data_converter::ImDataConverter` | Partial: `splitByFAIMSCV` only (D5), tier 1 against the executed SDK; skipped spectra, chromatograms and log records returned; NaN voltages refused; ranges computed on demand where the source's groups have none, which is why `FeatureFinderCentroided` can run on FAIMS input where the C++ tool exits 8 (`CPP-278`); [details](IM_DATA_CONVERTER_SUPPORT.md) |
| PROCESSING/FEATURE/FeatureOverlapFilter, extern/Quadtree | `processing::feature_overlap_filter` | Complete: every public member and the vendored quadtree, source mode (the uid-0 wipe and double merge kept), tier 1 on 78 executed cases bit for bit; errors leave the map unchanged and undefined source behaviour is refused. Package B11 added one native mode, `FaimsMergeFidelity::Corrected`, which answers `CPP-283` and is what the `FeatureFinderCentroided` tool runs; every faithful entry point is unchanged; [details](FEATURE_OVERLAP_FILTER_SUPPORT.md) |
| FEATUREFINDER/FeatureFinderAlgorithmPickedHelperStructs | `analysis::feature_finder_picked::helper_structs` | Partial: every member of seeds, mass traces, intensity profiles and isotope patterns, tier 1 for the computational members; stays partial until kernel hull merging keeps the source's first signed-zero m/z; [details](FEATURE_FINDER_PICKED_HELPER_STRUCTS_SUPPORT.md) |
| APPLICATIONS/TOPPBase, ParameterInformation (OpenMS4-cli package) | `cli` | Partial: lifecycle closure parts 1 and 2 with TOPPBase exit codes for every tool (D3), `-write_ini` parity, content-based input formats, usage on stderr and the DataProcessing retrofit (D4), tier 1 against the executed SDK; JSON INI files, `-log` and `checkParam_` warnings remain; [details](TOPP_CLI_SUPPORT.md) |
...
- `concept::progress_logger` supplies all core progress modes/backends and CPU/wall timing; see [progress support](PROGRESS_LOGGER_SUPPORT.md); ranges follow the Release build (inverted ranges accepted).
