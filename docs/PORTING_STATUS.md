...
| FEATUREFINDER/FeatureFinderAlgorithmPicked | `analysis::feature_finder_picked::{algorithm, extension, fitting, resolution, scoring, seeds}` | Partial: the whole algorithm is ported and `run()` produces features end to end — parameters, input checks, the scores, pattern precalculation, seed selection, mass-trace extension, the isotope fit, the trace fits, the quality checks, cropping, abort bookkeeping and overlap resolution. Tier 1: the seed stage bit for bit in six of seven executed configurations, and the feature stage against the C++ Release build (8 features, 30 hulls and 120 hull points in the default `FeatureFinderCentroided_1` run, 24 seeds in its `-seeds` run and 1,054 hull points under `-debug 5`, bit-identical m/z and hull points, fitted fields within 1e-9 relative). `min_spectra = 1` now follows the source and a changed isotope abundance computes the intended override, per the lead's decisions; partial because `write_debug` is refused, so `writeFeatureDebugInfo_` and `abort_reasons_` are not ported, and because a zero-width retention-time range is refused where the Release build returns an empty map; [details](FEATURE_FINDER_PICKED_SUPPORT.md) |
