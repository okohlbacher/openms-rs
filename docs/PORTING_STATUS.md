...
| FORMAT/MzMLFile | `format::mzml` | Optional bounded subset, including spectrum- and scan-level mobility and unit-bearing ion-mobility arrays ([mobility](MZML_MOBILITY_SUPPORT.md)) and opt-in source reading of dangling software and data-processing references ([headers](MZML_HEADER_SUPPORT.md)); see [detailed support](MZML_SUPPORT.md) |
| FORMAT/FileInfo | `format::file_info::{FileInfo, Options, FileInfoResult}` | Partial: the result model, `run`, `toText` and `toTSV`, with the DTA, DTA2D, mzML and featureXML branches and `-m`, `-p` and `-s`, tier 1 on 34 executed report pairs; `-i`, `-d` and `-c` are ported (A6-FILEINFO), tier 1 on 59 executed Release-build cases of which 38 compare both reports byte for byte; `-v` and the consensusXML, identification, FASTA, mzXML, mzData and trafoXML branches stay refused until packages A7 and A8. The `FileInfo` TOPP tool wraps it, with tier-1 evidence from the four retained upstream registrations and 17 executed product-SDK cases; [details](FILE_INFO_SUPPORT.md), [tool](TOPP_FILE_INFO_SUPPORT.md) |
