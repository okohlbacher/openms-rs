...
Levenberg-Marquardt crate failed its gate, so the Eigen transcription stays. The
trace fitters (B4, B5) and the wave-3 tool packages are not merged; see
...
| FORMAT/FileInfo | `format::file_info::{FileInfo, Options, FileInfoResult}` | Partial: the result model, `run`, `toText` and `toTSV`, with the DTA, DTA2D, mzML and featureXML branches and `-m`, `-p` and `-s`, tier 1 on 34 executed report pairs; `-i`, `-d`, `-c`, `-v` and the consensusXML, identification, FASTA, mzXML, mzData and trafoXML branches refused until packages A6 to A8; [details](FILE_INFO_SUPPORT.md) |
...
| FEATUREFINDER/FeatureFinderAlgorithmPicked | `analysis::feature_finder_picked::{algorithm, scoring, seeds}` | Partial, front half: parameters, input checks, intensity, trace and isotope-pattern scores, pattern precalculation and seed selection, tier 1 bit for bit in 7 executed configurations (overall scores correctly rounded); `run` returns `Unsupported` after seed selection until package B7; `min_spectra = 1` and changed isotope abundances refused pending a decision; [details](FEATURE_FINDER_PICKED_SUPPORT.md) |
