...
| FORMAT/MzMLFile | `format::mzml` | Optional bounded subset, including spectrum- and scan-level mobility and unit-bearing ion-mobility arrays ([mobility](MZML_MOBILITY_SUPPORT.md)); see [detailed support](MZML_SUPPORT.md) |
...
| APPLICATIONS/TOPPBase, ParameterInformation (OpenMS4-cli package) | `cli` | Partial: lifecycle closure part 1 with TOPPBase exit codes for every tool (D3), tier 1 against the executed SDK; `-write_ini` parity, content-based formats and the DataProcessing retrofit remain; [details](TOPP_CLI_SUPPORT.md) |
