...
* **The source's own reader accepts these files:** `FileInfo` exits 0 and prints `Error: unregistered source file reference sf_sp_1.` for the spectrum references (`MzMLHandler.cpp:896-906`) and says nothing at all about the dangling `dataProcessingRef`, which CPP-262 already covers: `processing_[ref]` resolves with `std::map::operator[]`, so an unknown id silently yields an empty history. Measured directly in wave 8 with a reader probe rather than inferred from `FileInfo`'s exit code (`../oracle/reader-roundtrip/probe_source_refs.cpp`, `logs/probe_06.log`): a spectrum's dangling `sourceFileRef` leaves the record the `SourceFile()` its construction gave it and warns **once per occurrence**; a chromatogram's default-constructs, inserts and sets an empty `SourceFile` silently (`:937-941`); a scan's and a precursor's set `source_file_name` and `source_file_path` to empty strings, also silently (`:1131-1137`, `:1313-1318`, `:1339-1344`). The `operator[]` insertions never reach `exp_->getSourceFiles()`, because that copy runs when `</sourceFileList>` closes, before the run element.

**Rust handling:** `ReferencePolicy`, in `src/format/ms_data_writing_consumer.rs`. `Checked`, the default, refuses the record with `Error::Unsupported` rather than writing a reference that cannot resolve. `SourceDangling` reproduces the source in the source's own `sf_sp_<s>`, `dp_sp_<s>` and `dp_sp_<s>_bi_<m>` spelling — this writer's own identifiers are zero-padded positions in one namespace, so reusing them would make the reference *resolve*, to the wrong entry, instead of dangling — and `PeakPickerHiRes -processOption lowmemory` selects it, because refusing stops that mode after five records on any `FileMerger` output. On the 110-record file the port's low-memory output carries the same 105 dangling identifiers as the C++ one and the same decoded content. Wave 8 closed the one gap this paragraph used to record. `SourceDangling` now decides "differs from the first record's" with element-wise `Arc::ptr_eq`, which is the source's own comparison on the same model: the reader resolves every `dataProcessingRef` through one registry entry per identifier and clones the `Arc` handles out of it, exactly as `processing_[ref]` clones `shared_ptr`s. On the `dp_sp_0`/`dp_sp_1` fixture above, now committed as `tests/data/peak_picking/PeakPickerHiRes_dupdp_input.mzML`, the port's low-memory output carries the same `dp_sp_1` and `dp_sp_2` the source writes, and is byte-identical to its own output on the unmodified fixture, as the C++'s is. What is **not** reproduced is the whole-document trigger: this port's `mzml::write` deduplicates by content on both halves, so it declares one `dataProcessing` and writes no record reference where `MzMLFile::store` declares one and dangles two. Reproducing that would mean emitting invalid mzML by default from every write path in the crate; it is recorded as a divergence and pinned by `the_whole_document_writer_still_deduplicates_by_content` in `tests/mzml_source_file_round_trip.rs`. The reader side is now consistent with the writer side: the port reads a file carrying these references back under `mzml::ReadOptions::source_dangling_references` (decision D14), which is what the source's reader does, and still refuses it by default. See [src/format/ms_data_writing_consumer.rs](src/format/ms_data_writing_consumer.rs), [tests/ms_data_writing_consumer.rs](tests/ms_data_writing_consumer.rs), [MS_DATA_WRITING_CONSUMER_SUPPORT.md](docs/MS_DATA_WRITING_CONSUMER_SUPPORT.md) and native difference 12 of [TOPP_PEAK_PICKER_HI_RES_SUPPORT.md](docs/TOPP_PEAK_PICKER_HI_RES_SUPPORT.md).
...
so `it_aad` becomes `(-inf) + (+inf) = NaN`, `FORMAT/FileInfo.cpp:2323` divides it by `cm.size()` and `:2327` pushes the NaN into `it_aad_by_cfs`. That vector is handed to `Math::SummaryStatistics<vector<double>>` at `:2371`, whose constructor calls `sort(data.begin(), data.end())` at `StatisticFunctions.h:948`.
